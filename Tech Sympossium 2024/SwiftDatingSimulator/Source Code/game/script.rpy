...
# player stat change functions
        ...
        def check_password(self, Hope): 
            ...
            Youre = True
            Having=len(Hope)
            if Having > 0 and Having < 24 and Having > 11 and Having < 13:
                Fun=[]
                for glaresIntensely in Hope:
                    Fun.append(ord(glaresIntensely))
            else:
                return False
            if not (Hope[6].isdigit() or Hope[8].isdigit()):
                return False

            if Hope[3] != "v":
                Youre = False
            if (Fun[8] ^ Fun[11] != False):
                Youre = False
            if Fun[5] != 0x68:
                Youre = False
            if Fun[2] != int('1110101', 2):
                Youre = False
            if Hope[0] != alphabet[(alphabet.find("j") + (pow(ord("e"),932)) + 4) % len(alphabet)]:
                Youre = False
            if Fun[7] != (int(Hope[6]) ** 4) + (int(Hope[8]) * 7):
                Youre = False
            if Fun[4] != 40:
                Youre = False
            if Fun[6] != (1+1)*(pow(2,6))-2*(pow(2,5))-0b1101:
                Youre = False
            if (''.join([Hope[11],Hope[10],Hope[9]]).upper()[::-1] != '\x41'+'\x42'+'\x35'):
                Youre = False
            if Hope[1] != hashlib.sha256(b"a").hexdigest()[6]:
                Youre = False

            return Youre

        def generate_flag(self, key):
            alphabet = "!\"#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ"+ \
            "[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~"

            flag = ""
            flag += alphabet[(alphabet.find(key[0]) + 35) % len(alphabet)]
            flag += alphabet[(alphabet.find(key[1]) + 3) % len(alphabet)]
            flag += alphabet[(alphabet.find(key[2]) - 1) % len(alphabet)]
            flag += alphabet[(alphabet.find(key[3]) + 27) % len(alphabet)]
            flag += alphabet[(alphabet.find(key[4]) + 79) % len(alphabet)]
            flag += alphabet[(alphabet.find(key[5]) + 23) % len(alphabet)]
            flag += alphabet[(alphabet.find(key[6]) + 4) % len(alphabet)]
            flag += alphabet[(alphabet.find(key[7]) - 12) % len(alphabet)]
            flag += alphabet[(alphabet.find(key[8]) + 14) % len(alphabet)]
            flag += alphabet[(alphabet.find(key[9]) + 39) % len(alphabet)]
            flag += alphabet[(alphabet.find(key[10]) - 15) % len(alphabet)]
            flag += alphabet[(alphabet.find(key[11]) + 63) % len(alphabet)]
            ...
            label ctf:
                menu:
                    ...
                    "That sounds great!":
                        ...
                        jump prologue2
                    "NO! I'm talking about the Tech Symposium CTF flag! Where is it!?":
                        player.fullname "I'm supposed to find a flag for a Tech Symposium CTF challenge in this game. It's supposed to be some specific string that's encased in the curly braces of 'ts\{\}'. Have you seen anything like it?"
                        
                        menu:
                            chet.fullname "Hmmm, I HAVE heard of Tech Symposium before. I'm actually at the event right now!  But I still don't know what game you're talking about."
                            ...
                            "I'm talking about this game! CTFd said that this dating simulator would have a flag!":
                                ...
                                pike "*sigh* Is this for the SWIFT Tech Symposium CTF? This sounds like something Jacob would do. Give me a moment, I'll figure something out."
                                ...
                                show pike with dissolve:
                                    ...
                                    xanchor 0.5  
                                    yanchor 0.5 
                                pike "Hi [player_firstname], I'm back! I found Jacob in the SDC and I was able to get something from him.  He said it needed to still be decoded though."
                                pike "If you give me a key I think I can give you the flag!"
                                $ player_password = renpy.input("What's the password?")
                                pike "Here let me try"
                                $ change = player.check_password(player_password.strip())
                                if change == True:
                                    $ flag = player.generate_flag(player_password.strip())
                                    pike "Congrats, Here is the flag: ts\{[flag]\}"
                                ...
                                if change == False:
                                    pike "Sorry this key didn't work"
                                    "Having given the wrong key you feel disappointed in yourself, but don't want to cry in front of Dr. Pike"
                                    pike "It's alright, Keep practicing and one day you can teach that stuck up bastard, {color=#880000}{b}CHET{/b}{/color}, a lesson"
                                    hide pike with dissolve
                                    "You're surprised at Dr. Pike's language, but it fills you with ambition to take on the world!"
                                "{i}Have fun at Tech Sympossium!{i}"
                                jump ending
...
label ending:
    scene bluesky
    "Thank you for playing! That is all the content for now"
    "I modified this game to make a rev CTF, so go show support to 'terrance5' and 'chetapichart' on Discord for the original game and Alex Tselevich for the Orbital Weapons universe"
    "Message 'TID_RedLeaf' for any questions or thoughts for the CTF version"
    "Ryan for president!"
    return
